...
def load_machine(request):
    ...
    html = ''
    for m in machines:
        power = f"<i class='text-success fas fa-power-off'></i> <small>ON</small>" if m[
            'power_state'] == 'on' else "<i class='text-danger fas fa-power-off'></i> <small>OFF</small>"
...
def machine_commission(request, system_id=None):
    form = CommissionForm(request.POST or None, initial={
                          'system_id': system_id})
    if form.is_valid():
        data = form.clean()
        try:
